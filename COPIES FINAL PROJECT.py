...
def calc_SEC_GPA(array, course):
  ...
  df = pd.concat([df, pd.DataFrame(students_to_append)], ignore_index=True).drop_duplicates()
  df_help = pd.concat([df_help, pd.DataFrame(help_append)], ignore_index=True).drop_duplicates()
...
def begin(run):
    ...
    print("DF before filtering good students:", df)
    print("DF_HELP before filtering help students:", df_help)
    ...
    # Debug prints
    print("DF after filtering good students:", df_filtered_good)
    print("DF_HELP after filtering help students:", df_filtered_help)
    ...
    for index, row in df_filtered_help.iterrows():
        print("PRocessing student: ", row['Student'])
        file.write(row['Student'] + ' || ' + row['Courses'] + '\n')
        file.write(' ' * 80 + '\n')
